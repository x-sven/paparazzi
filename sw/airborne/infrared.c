...
void estimator_update_state_infrared( void ) {
  estimator_ir_phi  = atan2(ir_roll, ir_top) - ir_roll_neutral;
  
  estimator_ir_theta  = atan2(ir_pitch, ir_top) - ir_pitch_neutral;
  
  if (estimator_ir_theta < -M_PI_2)
    estimator_ir_theta += M_PI;
  else if (estimator_ir_theta > M_PI_2)
    estimator_ir_theta -= M_PI;
  
#if defined IR_CORRECTION_LEFT && defined IR_CORRECTION_RIGHT
    if (estimator_ir_phi >= 0) 
      estimator_ir_phi *= ir_correction_right;
    else
      estimator_ir_phi *= ir_correction_left;
#endif
    
#if defined IR_CORRECTION_UP && defined IR_CORRECTION_DOWN
    if (estimator_ir_theta >= 0)
      estimator_ir_theta *= ir_correction_up;
    else
      estimator_ir_theta *= ir_correction_down;
#endif
