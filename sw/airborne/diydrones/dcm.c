...
// Modified at Hochschule Bremen, Germany for use with paparazzi
...
void Matrix_update(void)
{
  ...
  Gyro_Vector[0]=Gyro_Scaled_X(read_adc(3)); //gyro x roll
  ...
  // learn first 10 sec the static gyro offsets olri 2010-07-23
  if ( cpu_time_sec > 0 && cpu_time_sec <= 10 ) {
    n++;
    gyro_offset[G_ROLL] += gyro[G_ROLL];
    gyro_offset[G_PITCH] += gyro[G_PITCH];
    gyro_offset[G_YAW] += gyro[G_YAW];
  }
  
  // copy razor gyro data to dcm data without offset cancalation
  Gyro_Vector[0] = -gyro_offset[G_ROLL]/n + gyro[G_ROLL];
  Gyro_Vector[1] = -gyro_offset[G_PITCH]/n + gyro[G_PITCH];
  Gyro_Vector[2] = -gyro_offset[G_YAW]/n + gyro[G_YAW];
  
  // define accel_offsets
  static float accel_offset[3] = { 0., 0., 0. };
  
  // copy razor acceleration data to dcm data with offset cancelation 
  Accel_Vector[0] = -accel_offset[ACC_X] + accel[ACC_X];
  Accel_Vector[1] = -accel_offset[ACC_Y] + accel[ACC_Y];
  Accel_Vector[2] = -accel_offset[ACC_Z] + accel[ACC_Z];
